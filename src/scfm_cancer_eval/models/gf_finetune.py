...
from scfm_cancer_eval.setup_path import resolve_under_models_path
import torch
from os.path import join
from transformers import BertForSequenceClassification, Trainer, TrainingArguments
from geneformer import DataCollatorForCellClassification
from scfm_cancer_eval.features.gf_extractor import GeneformerExtractor
import logging
...
from scfm_cancer_eval.models.train_utils import (
...
class GFFineTuneModel:
    ...
    def __init__(self, params): #model_dir, dict_dir, model_input_size=4096, output_dir):
        ...
        # Same rule as EmbeddingExtractor / GeneformerExtractor: relative paths live under MODELS_PATH.
        base_model_dir = resolve_under_models_path(str(self.params["model_dir"]).strip())
        self.model_dir = join(base_model_dir, self.model_name)
        self.dict_dir = resolve_under_models_path(str(self.params["dict_dir"]).strip())
    ...
    def tokenize_data(self, geneformer_reader, file_format):
        ...
        nproc = os.cpu_count()
        # Upstream Geneformer: if model_version=="V1", TranscriptomeTokenizer ignores custom gene_* paths
        # and loads bundled files (often Git LFS pointers), causing pickle errors (invalid load key 'v').
        # Same workaround as GeneformerExtractor: pass model_version="V2" so custom dict_dir paths are kept;
        # set special_token / model_input_size to match the real model series.
    ...
    def train(self, gf_data_loader):
        ...
        train_func_map = {'vote': self.__train_vote, 'avg': self.__train_avg, 'mil': self.__train_mil}
    ...
    def __train_avg(self, train_ids, test_ids, split_col, evaluate=True, viz=False, postfix=""):
        ...
        from scfm_cancer_eval.evaluation.eval import eval_classifier
        metrics_df, cls_report = eval_classifier(pred_df["label"], pred_df["pred"], pred_df["pred_score"],
                                                 estimator_name=self.model_name, label_names=self.label_names)
        if evaluate:
