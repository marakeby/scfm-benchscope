...
# Same heuristic as scFoundation: Ensembl-style ``var_names`` (human ENSG…).
...
class SCimilarityExtractor(EmbeddingExtractor):
    MODELS_PATH_KEYS = frozenset({"model_path", "gene_name_id_dict"})
    ...
    def _maybe_apply_ensembl_to_symbols(self, adata):
        """Return ``(adata, did_map)`` after optional Ensembl → HGNC symbol rename for ``align_dataset``."""
        mapping = self.params.get("gene_name_id_dict")
        if not mapping:
            return adata, False
        force = bool(self.params.get("force_ensembl_symbol_mapping", False))
        if not force and not _var_names_majority_ensembl(adata.var_names):
            self.log.info(
                "SCimilarity: gene_name_id_dict is set but var_names are not majority Ensembl-like; "
                "skipping map (set force_ensembl_symbol_mapping: true to override)."
            )
            return adata, False
        self.log.info(
            "SCimilarity: renaming Ensembl var_names to gene symbols via gene_name_id_dict "
            "before align_dataset."
        )
        shim = SimpleNamespace(adata=adata, log=self.log)
        H5ADLoader.map_ensembl_to_gene_names(shim, mapping)
        return shim.adata, True
    ...
    def preprocess_h5ad(self, h5ad_path_data , reorder: bool = True, norm: bool = True):
        ...
        adata, did_map = self._maybe_apply_ensembl_to_symbols(adata)
        if did_map and getattr(self, "data_loader", None) is not None:
            self.data_loader.adata = adata
    ...
    def fit_transform(self, data_loader):
        ...
        adata = self.preprocess_h5ad(data_loader.adata, reorder=True, norm=True)
