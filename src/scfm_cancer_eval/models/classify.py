...
from scfm_cancer_eval.run.run_utils import get_split_dict
from scfm_cancer_eval.utils.saving import save_supervised
    ...
    def _plot_sklearn_learning_curve_log_loss(
        ...
        *,
        ...
        if isinstance(X_train, pd.DataFrame):
            ...
            X_use = X_train.to_frame()
        else:
            X_arr = X_train.toarray() if hasattr(X_train, "toarray") else np.asarray(X_train)
            if X_arr.ndim == 1:
                X_arr = X_arr.reshape(-1, 1)
            elif X_arr.ndim > 2:
                X_arr = X_arr.reshape(X_arr.shape[0], -1)
            X_use = pd.DataFrame(X_arr)
...
#             return mean_emb.loc[labels.index], labels['label']
    ...
    def __train_mil(self, adata_train, adata_test, evaluate=False, viz=False):
        ...
        if viz:
            try:
                from scfm_cancer_eval.models.attention_viz import (
